...
static ptrLTENR_EPC_HLR LTENR_EPC_FindHLR(NETSIM_ID d, NETSIM_ID in, NETSIM_ID dest)
{
...
static NETSIM_ID LTENR_EPC_HLR_FindOutInterface(NETSIM_ID d, NETSIM_ID dest)
{
	NETSIM_ID i;
	for (i = 0; i < DEVICE(d)->nNumOfInterface; i++)
	{
...
void LTENR_EPC_NetworkOut()
{
	...
	if (!outIn)
	{
		fnNetSimError("Device %d is not connected to EPC %d\n", dest, d);
		fn_NetSim_Packet_FreePacket(packet);
		return;
	}
