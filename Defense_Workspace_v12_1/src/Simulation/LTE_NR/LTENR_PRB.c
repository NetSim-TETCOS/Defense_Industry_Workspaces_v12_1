...
static void LTENR_PRB_UpdateRank_FAIR(ptrLTENR_UESCHEDULARINFO list, ptrLTENR_SCHEDULARINFO schedularInfo) {
...
static void LTENR_PRB_AllocatePRB(ptrLTENR_SCHEDULARINFO schedularInfo, ptrLTENR_UESCHEDULARINFO* curr, UINT totalPRBAvailable) {
	...
	*curr = current;
	...
	while (current && totalPRBAvailable > 0) {
		UINT PRBallocation = (UINT)ceil(current->bufferSize * 8.0 / (current->bitsPerPRB * schedularInfo->symbolPerSlot));
	...
	if (schedularInfo->schedulingType == LTENR_MAC_SCHEDULING_FAIR_SCHEDULING)
		LTENR_PRB_UpdateRank_FAIR(current1, schedularInfo);
