...
static ptrLTENR_PDCPENTITY LTENR_PDCP_FindEntity(ptrLTENR_PDCPVAR pdcp,
												 NETSIM_ID enbId,
												 NETSIM_ID enbIf,
												 NETSIM_ID ueId,
												 NETSIM_ID ueIf)
												...
												NETSIM_ID remote)
{
	...
	if (isUE(d, in))
		return LTENR_PDCP_FindEntity(pdcp, remote, rin, d, in);
	else if (isGNB(d, in))
		return LTENR_PDCP_FindEntity(pdcp, d, in, remote, rin);
	else
	{
												...
												NETSIM_ID ueIf)
{
	...
	ptrLTENR_PDCPENTITY entity = LTENR_PDCP_CreateNewEntity(gnbId, gnbIf, ueId, ueIf);
...
void LTENR_PDCP_START_DISCARD_TIMER(ptrLTENR_PDCPENTITY entity)
{
	...
	pevent.nSubEventType = LTENR_SUBEVENT_PDCP_DISCARDTIMER;
...
static void LTENR_PDCP_START_TREORDERING(ptrLTENR_PDCPENTITY entity)
{
	...
	pevent.nInterfaceId = pstruEventDetails->nInterfaceId;
	pevent.nProtocolId = MAC_PROTOCOL_LTE_NR;
	pevent.nSubEventType = LTENR_SUBEVENT_PDCP_TREORDERINGTIMER;
	pevent.szOtherDetails = entity;
												...
												PUINT32 count)
{
	*recv_sn = hdr->SN;
	...
	if (*recv_sn < LTENR_PDCP_SN(entity->RX_DELIV) - LTENR_PDCP_WINDOW_SIZE)
		* recv_hfn = LTENR_PDCP_HFN(entity->RX_DELIV) + 1;
	else if (*recv_sn >= LTENR_PDCP_SN(entity->RX_DELIV) + LTENR_PDCP_WINDOW_SIZE)
		* recv_hfn = LTENR_PDCP_HFN(entity->RX_DELIV) - 1;
	else
		*recv_hfn = LTENR_PDCP_HFN(entity->RX_DELIV);
...
void fn_NetSim_LTENR_PDCP_TransmitPDCPSDU()
{
	...
	ptrLTENR_PDCPENTITY entity = LTENR_PDCPENTITY_GET(pdcp, d, in, packet->nReceiverId);
	if (!entity)
	{
...
void fn_NetSim_LTENR_PDCP_ReceivePDCPSDU()
{
	...
	ptrLTENR_PDCPVAR pdcp = LTENR_PDCP_GET(d, in);
	ptrLTENR_PDCPENTITY entity = LTENR_PDCPENTITY_GET(pdcp, d, in, packet->nTransmitterId);
	if (!entity)
	{
		fnNetSimError("No PDCP entity is found between device %d and %d\n"
					  "Dropping packet in function %s\n",
...
#pragma region PDCP_TOBECHECKEDANDDELETED
#pragma message(__LOC__"Please check the logic of PDCP reordering windows \nThen delete the below section. Not going to affect the phase 1 result as there is no error in 5G link")
	{
		LTENR_HDR_FREE_FROM_PACKET(packet, PDCPHDRID);
		LTENR_PDCP_SendUp();
		return;
							...
							bool isAssociated)
{
	...
	{
#pragma message(__LOC__"Implement dissociation for PDCP in function "__FUNCTION__)
	}
}
