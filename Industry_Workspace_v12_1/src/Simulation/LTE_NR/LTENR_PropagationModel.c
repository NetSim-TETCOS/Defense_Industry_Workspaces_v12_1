	...
	switch (info->currentScenario)
	{
		...
		case LTENR_SCENARIO_UMA:
		{
			...
			else
			{
				...
				LOS_probability = ((18 / distance2D_out) + (exp(-(distance2D_out / 63)) * (1 - (18 / distance2D_out))))
					* (1 + (chUT * (5 / 4) * pow((distance2D_out / 100), 3) * exp(-(distance2D_out / 150))));
			}
		...
		case LTENR_SCENARIO_UMI:
		{
			if (distance2D_out <= 18)
				...
				LOS_probability = ((18 / distance2D_out) + (exp(-(distance2D_out / 36)) * (1 - (18 / distance2D_out))));
...
static double calculate_pathloss_only(ptrLTENR_PROPAGATIONINFO info)
{
	...
	switch (info->currentScenario)
	{
		case LTENR_SCENARIO_RMA:
		{
			...
			PL1 = (20 * log10(40 * PI * distance3D * fc / 3)) + fmin((0.03 * pow(h, 1.72)), 10) * log10(distance3D)
				- fmin((0.044 * pow(h, 1.72)), 14.77) + (0.002 * log10(h) * distance3D);

			PL2 = (PL1 * distanceBP) + (40 * log10(distance3D / distanceBP));

			if (10 <= distance2D <= distanceBP)
				pathloss_RMa_LOS = PL1;
			else if (distanceBP <= distance2D <= 1000)
			...
			{
				PL3 = 161.04 - (7.1 * log10(W)) + (7.5 * log10(h)) - ((24.37 - 3.7 * pow((h / hBS), 2)) * log10(hBS))
					+ ((43.42 - 3.1 * log10(hBS)) * (log10(distance3D) - 3)) + (20 * log10(fc))
		...
		case LTENR_SCENARIO_UMA:
		{
			...
			PL2 = 28.0 + (40 * log10(distance3D)) + (20 * log10(fc)) - (9 * log10(pow(distanceBP, 2) + pow((hBS - hUT), 2)));

			if (10 <= distance2D <= distanceBP)
				pathloss_UMa_LOS = PL1;
			else if (distanceBP <= distance2D <= 5000)
		...
		case LTENR_SCENARIO_UMI:
		{
			...
			PL2 = 32.4 + (40 * log10(distance3D)) + (20 * log10(fc))
...
static double calculate_O2I_penetrationloss(ptrLTENR_PROPAGATIONINFO info)
{
	...
	switch (info->propagationConfig->O2I_model)
	{
		case LTENR_O2IMODEL_LOW_LOSS:
		{
			...
			PL_in = 0.5 * info->dist2Dindoor;

			std_p = 4.4;

			O2Iloss = PL_tw + PL_in + normal_distribution(0, pow(std_p, 2));
			return O2Iloss;
		}
		break;
...
static void recalculate_localvar(ptrLTENR_PROPAGATIONINFO info)
{
	...
	switch (info->currentScenario)
	{
		case LTENR_SCENARIO_RMA:
		{
			...
			info->dist3D = calculate_distance3D(info->dist2D, 
												info->propagationConfig->gNB_height, 
...
void LTENR_Propagation_TotalLoss(ptrLTENR_PROPAGATIONINFO info)
{
	...
	switch (info->propagationConfig->channel_character)
	{
		...
		case LTENR_CHANNELCHAR_PATHLOSS_ONLY:
		{
			...
			info->dTotalLoss = info->dPathLoss;
			info->dO2ILoss = 0;
			info->dShadowFadingLoss = 0;
		}
		break;
		...
		case LTENR_CHANNELCHAR_PATHLOSS_SHADOWFADING:
		{
			...
			info->dTotalLoss = info->dPathLoss + info->dShadowFadingLoss;
			info->dO2ILoss = 0;
