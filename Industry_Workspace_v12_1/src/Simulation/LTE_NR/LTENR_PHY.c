...
void fn_NetSim_LTENR_GNBPHY_Init(NETSIM_ID gnbId, NETSIM_ID gnbIf)
{
	...
	for (i = 0; i < phy->ca_count; i++) {
		...
		pstruEventDetails->szOtherDetails = i;
...
double LTENR_PHY_Send_SINR_to_RRC(ptrLTENR_GNBPHY phy) {
	...
	double sinr = 0;
	for (i = 0; i < phy->ca_count; i++) {
		 sinr =+ phy->associatedUEPhyInfo->propagationInfo[i]->SNR_db;
	}
...
double LTENR_PHY_Send_RSRP_to_RRC(ptrLTENR_GNBPHY phy) {
	...
	double rsrp = 0;
	for (i = 0; i < phy->ca_count; i++) {
		rsrp =+ phy->associatedUEPhyInfo->propagationInfo[i]->rxPower_dbm;
	}
...
{
	//Downlink
	info->downlinkAMCInfo[CA_ID]->SpectralEfficiency = info->propagationInfo[CA_ID]->spectralEfficiency;
	...
	//Uplink
	info->uplinkAMCInfo[CA_ID]->SpectralEfficiency = info->propagationInfo[CA_ID]->spectralEfficiency;
